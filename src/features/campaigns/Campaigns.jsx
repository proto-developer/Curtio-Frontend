...
import { SHORTENER_DOMAIN } from "@/config/shortener";
import { listUrls, createUrl, deleteUrl, toggleUrl, updateUrlCampaigns, deleteCampaign } from "@/api/urls";
...
export default function Campaigns() {
  ...
  async function performDelete() {
    ...
    setError("");
    try {
      const data = await deleteUrl(slug);
      if (data.success) {
  ...
  async function performDeleteCampaign() {
    ...
    setError("");
    try {
      const data = await deleteCampaign(name);
      if (data.success) {
        await fetchUrls();
  ...
  async function performRemoveLinkFromCampaign() {
    ...
    try {
      ...
      const data = await updateUrlCampaigns(linkObj.slug, updatedCampaigns);
      if (data.success) {
        await fetchUrls();
  ...
  async function handleToggle(id, slug) {
    setError("");
    try {
      const data = await toggleUrl(slug);
      if (data.success) {
        setLinks((prev) =>
          prev.map((l) =>
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      if (useExistingLink) {
        ...
        const data = await updateUrlCampaigns(targetLink.slug, existingCampaigns);
        if (data.success) {
          await fetchUrls();
          setDestUrl("");
          setCampaignName("");
        ...
        <main className="flex-1 min-w-0 lg:ml-72 xl:ml-80 px-2 lg:px-4 sm:px-6 md:px-8 py-6 md:py-8 space-y-3 lg:space-y-6">
          ...
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {/* Hidden on phones/small tablets, where it only ever rendered as
                  a meaningless one-line ellipsis. Shown from md up, where there
                  is room for the full sentence on a single line. */}
              <p className="hidden md:block text-slate-500 text-xs sm:text-sm mt-0.5 truncate">
                {selectedCampaign
